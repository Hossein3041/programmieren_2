// Aufgabe 1: Verändern Sie Ihr Programm zur Darstellung der Rechtecke derart, dass Sie die Maus
// über einem beliebigen Rechteck positionieren und durch Drehen des Mausrades die Farbe dieses Rechtecks ändern.
package uebung;
import java.awt.*;
import javax.swing.*;
import java.awt.event.*;
import java.util.Scanner;
class U05_AFG01_Model{
    public int sizeX, sizeY, m, n, space = 2;
    U05_AFG01_Model(int sizeX, int sizeY, int m, int n){
...
class U05_AFG01_View extends JComponent{
    private U05_AFG01_Model m_Model;
    U05_AFG01_View(U05_AFG01_Model model){
        this.m_Model = model;
        this.setOpaque(true);
    ...
    public void paintComponent(Graphics g){
        ...
        for(int i = 0; i < m_Model.n; ++i){
            for(int j = 0; j <  m_Model.m; ++j){
                ...
                g.setColor(m_Model.newColor());
                g.fillRect(x, y, rectWidth, rectHeight);
            }
        }
    }
    public void changeRectColor(int index){
        int row = index / m_Model.m;
        int column = index % m_Model.m;
        ...
        int rectPosX = column * (rectWidth + m_Model.space);
        int rectPosY = row * (rectHeight + m_Model.space);

        Graphics g = getGraphics();
        g.setColor(m_Model.newColor());
        g.fillRect(rectPosX, rectPosY, rectWidth, rectHeight);

        g.dispose();
    }
}
class U05_AFG01_Controller{
    private U05_AFG01_Model m_Model;
    private U05_AFG01_View m_View;
    private JFrame frame = new JFrame();
    private ImageIcon icon;
    private JButton closeButton;
    U05_AFG01_Controller(){
        Scanner sc = new Scanner(System.in);
        System.out.println("Erst Fenstergröße, dann Anzahl Rechtecke + Zeile: ");
        m_Model = new U05_AFG01_Model(sc.nextInt(), sc.nextInt(), sc.nextInt(), sc.nextInt());
        m_View = new U05_AFG01_View(m_Model);
    ...
    private void addMouseWheelListenerToView(){
        m_View.addMouseWheelListener(e -> {
            ...
            if(indexRect != -1){
                m_View.changeRectColor(indexRect);
            }
        });
    }
    private int getIndexRect(int mousePosX, int mousePosY){
        ...
        for(int i = 0; i < m_Model.n; ++i){
            for(int j = 0; j < m_Model.m; ++j){
                ...
                if(mousePosX >= rectPosX && mousePosX <= rectPosX + rectWidth &&
                        mousePosY >= rectPosY && mousePosY <= rectPosY + rectHeight){
                    indexRect = i * m_Model.m + j;
                }
            }
        }
        return indexRect;
...
public class U05_AFG01 {
    public static void main(String[] args){
        new U05_AFG01_Controller();
    }
}
